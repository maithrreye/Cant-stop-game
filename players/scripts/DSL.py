...
class DSL:
    ...
    def containsNumber(action, number):
        ...
        """
        #print(str((action))) # <class 'str'>

        #print(str(isinstance(action, str))) #<class 'list'>
        if not isinstance(action, str):  #if action is type 'str' --> False or True
            if number in action:
                return True
        return False
    ...
    def actionWinsColumn(state, action):
        ...
        #print("Copy_state"+str(copy_state))
